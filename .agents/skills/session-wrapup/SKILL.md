...
此 skill 用于 NixOS 配置仓库 (`~/myNixOSConfig`) 的会话收尾；每次收尾都判断是否需要更新 memory 或 wiki，只记录持久决策和本次实际变化。
...
若用户仅要求总结或收尾，且未授权写入，先展示拟新增/更新的卡片、slug、理由和未知项，不要写入；总结或收尾本身不构成写入 memory 的授权。获得明确写入授权后，在已授权范围内连续完成可逆编辑、相关检查和必要修复。目标未定或跨越明确权限边界时再询问。部署、破坏性操作和提交最终确认仍按各自边界处理；对已获授权的 wiki 非小型写入或广泛审查不重复索取确认。移动、重命名、删除或覆盖已有内容仍须单独确认。
