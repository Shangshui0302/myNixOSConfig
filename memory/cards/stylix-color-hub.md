...
- **foot**：`host/hyprland/desktop.nix` 注入 stylix colors
- **hyprland**：`home/hyprland/hyprland.nix` 生成 `hypr/stylix-colors.lua`，hyprland.lua `pcall(require, "stylix-colors")`
- **niri**：`home/hyprland/niri.nix` 生成 `niri/stylix-colors.kdl`，config.kdl `include optional=true` 引入
...
## How to apply
- 改全局配色：只改 `host/hyprland/stylix.nix` 的 `image`（壁纸），rebuild 生效；`polarity` 控制暗色（当前 dark）。
