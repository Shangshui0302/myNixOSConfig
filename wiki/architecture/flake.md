...
- Home Manager 侧：`useGlobalPkgs`/`useUserPackages` 开启，`backupFileExtension = "backup"`，`home-manager.users.lishangshui` 指向 `home/default.nix`，并通过 `extraSpecialArgs` 再次传入 `inputs`。
...
Modules --> HM["home-manager.users.<user> = import ./home/default.nix"]
HM --> ExtraArgs["extraSpecialArgs={inherit inputs}"]
ExtraArgs --> End(["生成系统配置"])
```

各子模块的组合方式：`host/default.nix` 集中导入 `boot`、`hardware`、`locale`、`nix`、`users`、`network`、`services`、`desktop`、`greeter`、`gaming`、`containers`、`sops`；`home/default.nix` 导入 Git、主题、环境、开发、生产力、娱乐等模块，并启用 nix-flatpak 用户模块。
...
Root --> Home["home/default.nix"]
Root --> Overlays["overlays/default.nix"]
Host --> SOPS["host/sops.nix"]
...
- 缓存加速：在 `host/nix.nix` 配置多个 substituters（官方 + 镜像源），并启用 `nix-command`、`flakes` 实验特性；`nh` 自动清理旧代际以维持磁盘空间。
