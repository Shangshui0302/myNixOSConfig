...
title: 深色模式与动态配色
category: desktop
tags: [darkmode, darkman, matugen, gtk, qt, portal, btop, yazi, zellij, obsidian, vscode]
updated: 2026-09-08
---
...
`theme-apply` 将壁纸文件内容的 SHA-256、Matugen 版本、模板配置和缓存格式组合成缓存键，产物存放在 `~/.cache/wallpaper-colors/cache/<key>/`。每次壁纸变化只在缓存未命中时运行 Matugen；一次运行会生成 btop 和 Yazi 深色主题、Zellij 深浅两套主题、VS Code 深浅两套色板、Obsidian 双模式重点色 snippet，以及 light/dark 两套 Qt、Kvantum、KDE、合成器和 Fcitx 产物。
...
宿主 GTK4 应用应在模式切换时立即跟随；GTK3 是否实时刷新取决于应用是否监听主题名变化。btop 始终重新读取深色主题文件；Yazi 始终读取深色 flavor，切换到新壁纸会更新其重点色；切换到新壁纸也会更新 Fcitx5 重点色并重启输入法；已缓存壁纸只复制现成产物。其他已运行 GTK 应用不保证热载颜色文件，应等旧进程退出后重新打开。Qt 应用同样通常需要重新打开。portal 查询失败时，先检查：
...
- [Yazi](../productivity/yazi.md) — Matugen 深色 flavor 与运行时路径
