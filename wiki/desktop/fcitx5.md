...
本文说明本仓库如何在 NixOS + Home Manager 下启用并集成 Fcitx5：引擎与主题管理、中文输入方案、Wayland/Hyprland 集成、候选窗定制与常见排障。配置分布在系统级（`host/desktop.nix`）与用户级（`home/env/hyprland.nix`、`home/theme.nix`）两处。
...
## 引擎与中文输入方案
中文能力由 Rime 引擎提供，在 `host/desktop.nix` 的 `fcitx5.addons` 中声明：

- `fcitx5-rime`，并通过 `rimeDataPkgs` 注入 `rime-ice`、`rime-moegirl`、`rime-zhwiki` 数据包。
- 前端与工具：`fcitx5-gtk`、`qt6Packages.fcitx5-chinese-addons`、`qt6Packages.fcitx5-configtool`、`kdePackages.fcitx5-qt`。

拼音、五笔、注音等具体方案由 Rime 数据与用户方案决定；上述数据包提供基础词库能力。如需新增或切换方案，在 Rime 数据层配置即可（不在本仓库内直接体现）。

## 主题与候选窗定制
ClassicUI 主题与候选窗在 `host/desktop.nix` 的 `fcitx5.settings.addons.classicui.globalSection` 中设置：
...
- 在 Hyprland 下，`home/env/hyprland.nix` 为 Fcitx5 提供 systemd user service 的崩溃自恢复策略（on-failure 重启），提升长期稳定性。
- `home/theme.nix` 负责字体/光标与 Portal 注册，确保候选窗渲染与深浅色联动正常。
