...
本文说明本仓库如何在 NixOS + Home Manager 下启用并集成 Fcitx5：引擎与主题管理、中文输入方案、Wayland/Hyprland 集成、候选窗定制与常见排障。配置分布在系统级（`host/hyprland/desktop.nix`）与用户级（`home/hyprland/hyprland.nix`、`home/theme-base.nix`）两处。
...
## 系统级配置要点
系统层在 `host/hyprland/desktop.nix` 中完成以下工作：
...
## 引擎与中文输入方案
中文能力由 Rime 引擎提供，在 `host/hyprland/desktop.nix` 的 `fcitx5.addons` 中声明：
...
## 主题与候选窗定制
ClassicUI 主题与候选窗在 `host/hyprland/desktop.nix` 的 `fcitx5.settings.addons.classicui.globalSection` 中设置：
