...
**配色（stylix）**：`host/hyprland/stylix.nix` 接入 stylix（`github:nix-community/stylix`）作为配色中枢，`config.lib.stylix.colors` 从壁纸取色。foot 配色在 desktop.nix 手工注入 stylix colors（本版本 stylix 无 foot target；**foot 1.27 颜色不接受 `#` 前缀，注入用无前缀 hex**）；hyprland/niri 配色手工注入（border 色）。GTK 保持 Material-Gnome、Qt 保持 qt5ct/breeze、Noctalia 面板保持 yamadaryou（stylix 对应 target 均显式关）。
