...
技术栈以 NixOS Unstable 为基础，采用 Hyprland（Wayland 窗口管理器，主力）+ niri（滚动平铺）+ GNOME（specialisation 变体，GDM 登录）三种桌面，Fcitx5 + Rime 中文输入法、Noctalia 主题系统；外部依赖由 Flake 统一锁定，敏感配置通过 SOPS + age 加密管理。
